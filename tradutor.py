...
def traduz_pedidos(pedido_csv: str,):
    ...
    with oracledb.connect(user=username, password=password, dsn=dsn) as connection:
        ...
        query = f"""SELECT DISTINCT I.DESCRICAO,
                           I.CODIGO,
                           I.TIPO,
                           I.SERVICO_ID,
                           CASE WHEN NOT EXISTS(SELECT 1
                                                 ...
                                                 WHERE CODIGO = I.CODIGO
                                                   AND IDIOMA_ID = 2) THEN 'es'
                           ELSE NULL
                            END AS FALTA_TRADUCAO_ES,
                           CASE WHEN NOT EXISTS(SELECT 1
                                                  FROM MENSAGEM_ERRO_ATM
                                                 WHERE CODIGO = I.CODIGO
                                                   AND IDIOMA_ID = 3) THEN 'en-US'
                           ELSE NULL
        ...
        with connection.cursor() as cursor:
            ...
            if not rows:
                erro = ("❌ Nenhum pedido encontrado ou pedido já traduzido.")
                return erro

            insert_sql = """INSERT INTO MENSAGEM_ERRO_ATM(ID,
                                                          CODIGO,
                                                          DESCRICAO,
                                                          SERVICO_ID,
                                                          IDIOMA_ID,
            ...
            for descricao, codigo, tipo, servico_id, falta_es, falta_en in rows:
                ...
                if falta_es:
                    ...
                    with connection.cursor() as c_id:
                        c_id.execute("""SELECT NVL(MAX(ID),0) + 1
                                          FROM MENSAGEM_ERRO_ATM""")
